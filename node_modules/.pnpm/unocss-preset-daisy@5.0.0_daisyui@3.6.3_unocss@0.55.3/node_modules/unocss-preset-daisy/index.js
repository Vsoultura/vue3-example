...
const replacePrefix = (css) => css.replace(/--tw-/g, '--un-');
// UnoCSS uses comma syntax
// var(--foo) / 0.1 -> var(--foo), 0.1
const replaceSlash = (css) => css.replace(/\) \/ /g, '), ');
const replaceSpace = (css) => 
// HSL
// 123 4% 5% -> 123, 4%, 5%
...
export const presetDaisy = (options = {}) => {
    ...
    for (const node of styles.flatMap(style => process(style).root.nodes)) {
        ...
        const rule = (isAtRule ? node.nodes[0] : node);
    ...
    return {
        ...
        theme: {
            colors: {
                ...
                base: Object.fromEntries(Object.entries(colors)
